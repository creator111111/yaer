...
## 宝箱获得 Tips 叠屏须串行 · 2026-09-23

详见：`Assets/Doc/执行文档/0923/宝箱获得Tips叠屏须串行_架构溯源报告.md`  
施工说明：`Assets/Doc/施工说明/0923/宝箱获得Tips叠屏须串行_施工说明.md`  
**侦探结论**：同帧二次 `OpenTipsForm` 在 `tipsFormLogic` 未赋前双开 `TipsPanel`；单面板队列未走到。推荐 GSM 方案 **A**（待开队列）。否决只 Delay 箱脚本 / 合并图。

| ID | 问题 | 决议 / 施工默认 | 状态 |
|----|------|-----------------|------|
| Q1 | 主因是双 Form 还是队列坏？ | **双 Form 竞态** | ✅ |
| Q2 | 修挂点？ | **TipsComponentGSM（A）** | ✅ **已施工** |
| Q3 | 只 Delay 村箱？ | **否** | ✅ |
| Q4 | 合并一张 Hp+Mp 图？ | **否** | ✅ |
| Q5 | Play 开箱 Hierarchy 是否 2 个 TipsPanel？ | 施工后对拍：应仅 1 | ⏳ 待 Play |

---

## 重新进入村长家黑屏 · 2026-09-23

详见：`Assets/Doc/执行文档/0923/重新进入村长家黑屏_架构溯源报告.md`  
施工说明：`Assets/Doc/施工说明/0923/重新进入村长家黑屏_施工说明.md`  
**侦探结论**：二进应 `ShouldPlay==false` → 默认 Close；卡黑优先 **CloseFormHideFade/HideFade 静默失败** 或 **换古莎 BlackPanel 残留叠层**；次查续聊未记档误 Defer。推荐 **A+B+C**。否决 Loading / 二进重播续聊。

| ID | 问题 | 决议 / 施工默认 | 状态 |
|----|------|-----------------|------|
| Q1 | 二进是否应再 Defer 续聊？ | **否**（已用则 false） | ✅ |
| Q2 | 主修？ | **A+B+C**（兜底揭幕+Close 加固+清残留） | ✅ **已施工** |
| Q3 | 开回 LoadingPanel？ | **否** | ✅ |
| Q4 | 卡黑时 Hierarchy 是否仍有 BlackPanel？ | Play 钉死 | ⏳ 待 Play |
| Q5 | 续聊档键二进是否仍未 used？ | Play 对拍 StoryTriggerCount | ⏳ 待 Play |

---

## 商店数量点击全选覆盖输入 · 2026-09-23

详见：`Assets/Doc/执行文档/0923/商店数量点击全选覆盖输入_架构溯源报告.md`  
施工说明：`Assets/Doc/施工说明/0923/商店数量点击全选覆盖输入_施工说明.md`  
**侦探结论**：累加因点击后全选未稳住（鼠标清选 + selection 透明）；盘上虽有 `onFocusSelectAll=1` 仍不够。推荐 Helper 强制全选 + 点选补 SelectAll。**0922 联合钳制公式不动**。

| ID | 问题 | 决议 / 施工默认 | 状态 |
|----|------|-----------------|------|
| Q1 | 累加主因？ | **未稳全选→末尾追加**（非钳制公式） | ✅ |
| Q2 | 是否关购买联合钳制？ | **否** | ✅ |
| Q3 | 恢复闪烁 caret？ | **否**（selection 可仍透明） | ✅ |
| Q4 | 最小挂点？ | Helper + 行 SelectAll 补刀 | ✅ **已施工**（后回归） |
| Q5 | Play：5 点后按 3 → 3 非 53？ | 施工后对拍 | ❌ 验收失败：输入后 DigitStrip 不变化 |
| Q6 | 不刷新根因？ | 见下节侦探报告 | ✅ 已出报告 |

---

## 商店数量输入 DigitStrip 不刷新（覆盖输入回归）· 2026-09-23

详见：`Assets/Doc/执行文档/0923/商店数量输入DigitStrip不刷新_架构溯源报告.md`  
提示词：`Assets/Doc/提示词/0923/商店数量输入DigitStrip不刷新_架构侦探提示词.md`  
施工说明：`Assets/Doc/施工说明/0923/商店数量输入DigitStrip不刷新_施工说明.md`  
**侦探结论**：第 3 轮 Validate 内 `SetTextWithoutNotify`+`\0` 绕开 onValueChanged，刷图链易断（B）；`max==0` 钳回易伪装成不变（C）。推荐 **改覆盖走 onValueChanged 收束 + RefreshDigitDisplay**；**禁**关联合钳制。Play 须分列 TMP vs DigitStrip。

| ID | 问题 | 决议 / 施工默认 | 状态 |
|----|------|-----------------|------|
| Q1 | 不变化属 A/B/C？ | **Play 分列**；代码偏 **B+C** | ⏳ 待 Play |
| Q2 | 可否关联合钳制？ | **否** | ✅ |
| Q3 | 覆盖目标是否保留？ | **是**（改实现，不撤产品） | ✅ |
| Q4 | 主修挂点？ | `ShopBuyRowQuantityInput` 覆盖重做 | ✅ **已施工** |
| Q5 | 延帧 SelectAll 是否再上？ | **否**（已证伤刷图） | ✅ |
| Q6 | Sync 静默失败？ | 一律 RefreshDigitDisplay；Find 失败打 Error/Ensure | ✅ **已施工** |

---

## 商店系统整修 · 按钮高亮与数量总价 · 2026-09-23

详见：`Assets/Doc/执行文档/0923/商店系统整修_按钮高亮与数量总价_架构溯源报告.md`  
施工说明：`Assets/Doc/施工说明/0923/商店系统整修_按钮高亮与数量总价_施工说明.md`  
**侦探结论**：三病都在行交互叠改。高亮=`transition=None` 手搓图；双击=整行 Pointer+两帧 Focus+Relay Up；总价超=`max≤0` 不钳 + 0.18s snapBack。整修 **R0 回滚交互 + 唯一 K1 全选覆盖**。0922 公式保留。禁再 Delay / Validate+`\0` / 关钳制。

| ID | 问题 | 决议 / 施工默认 | 状态 |
|----|------|-----------------|------|
| Q1 | 是否再叠第五套钩子？ | **否**；先删残骸 | ✅ **已施工** |
| Q2 | 覆盖金样？ | **K1**（全选 + 正常 onValueChanged） | ✅ **已施工** |
| Q3 | 关联合钳制？ | **否** | ✅ |
| Q4 | snapBack / max≤0 不钳？ | **删**；立刻钳，Total2 永不超金币 | ✅ **已施工** |
| Q5 | 手搓行高亮 / Transition.None？ | **删**；恢复 Prefab SpriteSwap | ✅ **已施工** |

---

## 商店货栏点击进输入并清空 · 2026-09-23

详见：`Assets/Doc/执行文档/0923/商店货栏点击进输入并清空_架构溯源报告.md`  
施工说明：`Assets/Doc/施工说明/0923/商店货栏点击进输入并清空_施工说明.md`  
**侦探结论**：长条有响应=Button 高亮，未 Activate；数字框没响应=DigitStrip 关射线后点击穿透到行 Button。统一 `BeginQuantityEdit`（清空含 0 + Activate）；两处都进输入。不动总价/高亮整修。失焦空→0 可接受。

| ID | 问题 | 决议 / 施工默认 | 状态 |
|----|------|-----------------|------|
| Q1 | 进输入清 0 还是全选留 0？ | **清空（含 0）** | ✅ **已施工** |
| Q2 | 长条是否也要进输入？ | **是** | ✅ **已施工** |
| Q3 | 改联合钳制/决定？ | **否**（另票） | ✅ |
| Q4 | 失焦空串？ | **空→0** | ✅ **已施工** |
| Q5 | 再 transition=None？ | **否** | ✅ |

---

## 购买数量皆变 0 与再点不清空 · 2026-09-23

详见：`Assets/Doc/执行文档/0923/购买数量皆变0与再点不清空_架构溯源报告.md`  
施工说明：`Assets/Doc/施工说明/0923/购买数量皆变0与再点不清空_施工说明.md`  
**侦探结论**：① 皆变 0 → 优先 `ResolveBuyMaxQuantity≡0` 边输边钳（Z1）；清空后买侧 Reclamp 为次（Z2）。② 再点不清空 → 已聚焦时 `onSelect`/行 Button 都不二次触发（R1）；须每次 PointerClick→`BeginQuantityEdit`。禁关钳制、禁要求先点别处。

| ID | 问题 | 决议 / 施工默认 | 状态 |
|----|------|-----------------|------|
| Q1 | 皆变 0 主因？ | Z2 减 Reclamp + 买侧单价口径统一；Z1 **未测 max 分解** | ⚠️ **验收失败** → 见下节复验 |
| Q2 | 再点不清空？ | **R1** Click 每次 Begin | ✅ **已施工** |
| Q3 | 关购买钳制？ | **否** | ✅ |
| Q4 | 再点是否必须先失焦？ | **否**；Click 每次 Begin | ✅ **已施工** |
| Q5 | Begin 清空后立刻 Invoke？ | 改为 `OnQuantityClearedForEdit`→只刷 Total2 | ✅ **已施工** |

---

## 购买输入仍变 0 · 有金无法买 · 复验热修 · 2026-09-23

详见：`Assets/Doc/执行文档/0923/购买输入仍变0_有金无法买_复验热修_架构溯源报告.md`  
施工说明：`Assets/Doc/施工说明/0923/购买输入仍变0_有金无法买_复验热修_施工说明.md`  
**侦探结论**：**G2** — `HpBall` held=10 → `stackRoom=0` → max≡0 → 输入皆钳 0；`[MenuMoney] gold=800` 证伪 G1。已挂 `[ShopBuyMax]`。施工：有空位时须 max≥1；满堆叠可辨，禁关钳制。

| ID | 问题 | 决议 / 施工默认 | 状态 |
|----|------|-----------------|------|
| Q1 | 有 800 金仍变 0？ | **G2 满堆叠**；非读金失败 | ✅ **已施工**（日志/Warning 可辨；公式保留） |
| Q2 | G1 菜单/商店金币分裂？ | **否**（同源 `GetPlayerGoldData`） | ✅ |
| Q3 | 关联合钳制？ | **否** | ✅ |
| Q4 | 满堆叠 UX？ | 钳 0 可留；`[ShopDebug] 堆叠已满无法再买` | ⚠️ **部分** → 见下节「提交再钳 + Tips」 |
| Q5 | 验收日志？ | `[ShopBuyMax]` + 须先 held&lt;10 | ✅ **已施工**（验收前置见施工说明） |

---

## 购买上限公式审计 · 卖正常买归 0 · 2026-09-23

详见：`Assets/Doc/执行文档/0923/购买上限公式审计_卖正常买归0_架构溯源报告.md`  
施工说明：`Assets/Doc/施工说明/0923/购买上限公式审计_卖正常买归0_施工说明.md`  
**侦探结论**：**T1** — `onValueChanged`→`TryClamp` 边输边写回；Play 栈 + `[ShopBuyMax] HpBall gold=800 … stackRoom=0 max=0`（B1）。产品改：**编辑自由、提交再钳**；买卖同步；max=0 须 Tips。禁关公式。

| ID | 问题 | 决议 / 施工默认 | 状态 |
|----|------|-----------------|------|
| Q1 | 编辑中是否钳？ | **否**；只 Sync 显示 | ✅ **已施工** |
| Q2 | 何时钳？ | 失焦 / Enter / 切商品行 / 决定前 Commit | ✅ **已施工** |
| Q3 | 联合 Reclamp 编辑行？ | **跳过**正在编辑行 | ✅ **已施工** |
| Q4 | max=0 本档？ | **B1** stackRoom=0；Commit 后 Console 可辨 | ✅ **已施工**（Tips 图集键未齐） |
| Q5 | 关 ResolveBuyMax？ | **否** | ✅ |

---

## 购买公式逐项查错 · 卖已正确买归 0 · 2026-09-23

详见：`Assets/Doc/执行文档/0923/购买公式逐项查错_卖已正确买归0_架构溯源报告.md`  
施工说明：`Assets/Doc/施工说明/0923/购买公式逐项查错_卖已正确买归0_施工说明.md`  
**侦探结论**：**F1** — Commit `[ShopBuyMax] HpBall gold=800 price=200 held=10 stackRoom=0 otherCost=0 max=0`；手算=日志。gold/price/otherCost 正常；**非公式算错**，是满堆叠空位门（读数正确）。卖 OK。禁改卖；禁关堆叠校验。

| ID | 问题 | 决议 / 施工默认 | 状态 |
|----|------|-----------------|------|
| Q1 | 哪项把 max 打成 0？ | **stackRoom=0**（held=10） | ✅ |
| Q2 | F2/F3/F4/F5/F6？ | **否** | ✅ |
