...
不要单独 Open `Village_Shop` 再 Play 来验收本需求（与 `Village_HomeScene4_NPC对话配置` 文档口径一致）。
...
- **方案 B**：商店只做 UI，叠在 `Village_HomeScene4`；ESC 由 HomeScene4 的 GSM 提供；`Village_Shop.unity` 继续当 Bake 沙盒。此时「接入关卡」= 门进 HomeScene4 + 开商店 UI，而不是换场到 Village_Shop。  
...
| ID | 问题 | 影响 | 建议默认 |
|----|------|------|----------|
| Q1 | 正式关卡入口是 **独立场景 `Village_Shop`**，还是 **`Village_HomeScene4` + 商店 UI**？（0629 文档曾写目标场景 HomeScene4） | 决定 VS-ESC-1～3 做不做换场 | ✅ **已拍板（2026-07-13）**：独立场景 `Village_Shop`，且为**纯 UI / 不生成玩家**。落地见 `0713/Village_KenMuNi1_Door_Shop换场Village_Shop纯UI_架构溯源与施工执行说明.md` |
