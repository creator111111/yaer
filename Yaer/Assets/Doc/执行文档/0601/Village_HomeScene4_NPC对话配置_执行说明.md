# Village_HomeScene4 — 可对话 NPC 配置执行说明
...
**目标场景**：`Assets/GameRes/Scenes/Village_HomeScene4.unity`  
...
### 2.1 当前 `Village_HomeScene4` 里已经有什么（静态阅读）
...
| `SceneManager` | 根节点 | `Village_House4SceneManager` | 场景能跑，但 `nowSceneName` 写的是 `Village_House4`（与场景文件名 `Village_HomeScene4` 不一致），**存档/换场核对时要注意**，与对话报错无直接关系 |
...
| 2 | 保存为 **`Assets/GameRes/Prefabs/Dialogue/Village_HomeScene4_Xiaer.prefab`**（名字自定，但记住 **不要** 放进 `HomeScene1` 子文件夹） |
...
5. 在 **`SimpleStoryTrigger`** 上改：  
   - **Story Prefab Name** = `Village_HomeScene4_Xiaer`（与 §4.1 prefab **文件名一致**，不含 `.prefab`）  
...
3. **Story Prefab Name** 填你在 §4.1 新建的名字（例如 `Village_HomeScene4_Xiaer`）。  
4. 保存场景。
...
1. 从 **`InitScene`** 启动游戏（不要单独 Open `Village_HomeScene4` 再 Play，否则 `GameManager` / 对话 UI 可能不全）。  
2. 换场或调试进入 **`Village_HomeScene4`**。  
...
| `Village_HomeScene4` 与 `Village_House4SceneManager` | 场景名与 `nowSceneName` 不一致，换场/存档需单独立项 |
| `SceneName.Village_HomeScene4` | `SceneName.cs` 中若未登记，换场工具链需补常量 |
...
| 日期 | 说明 |
|------|------|
| 2026-06-01 | 初版：针对 Village_HomeScene4 配置可对话 NPC；说明 HomeScene1Xiaer 与 Dialogue 路径不匹配导致的加载失败 |

**文档路径**：`Assets/Doc/执行文档/0601/Village_HomeScene4_NPC对话配置_执行说明.md`
