...
- 继续让 `Door_Shop` 指向 `Village_HomeScene23`（当前错误目标）
...
| **`SceneChangeDoor.NextSceneName`** | ❌ **`Village_HomeScene23`** | ✅ **`Village_Shop`** | **是** |
...
| 字段 | 操作 |
|------|------|
| `Scene Change Door` → **Next Scene Name** | 改为 **`Village_Shop`**（勿留 `Village_HomeScene23`） |
...
| Q-C | `Door_Shop` 与 `Village_HomeScene23` 是否仍保留另一扇门？ | 本门只进 `Village_Shop`；HomeScene4 用原有门，勿混用 |
