...
**`商店界面合层` 目前是「场景里摆着的 2D 精灵合层」，不是可点的 UI；要改成正式 UI 组件，需要新建 `ShopPanel.prefab`（UGUI + `ShopFormLogic`），把合层里的图逐层搬进 Canvas，并按已有商店脚本约定的节点名重命名——不要指望一键把 SpriteRenderer 自动变成 Button。**
