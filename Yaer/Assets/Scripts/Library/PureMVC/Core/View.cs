    ...
    public class View: IView
    {
        ...
        public virtual void RegisterMediator(IMediator mediator)
        {
            ...
            if(mediatorMap.TryAdd(mediator.MediatorName, mediator))
            {
                ...
                {
                    // Create Observer referencing this panelMediator's handleNotification method
                ...
                // alert the panelMediator that it has been registered
                mediator.OnRegister();
            }
        }
        ...
        {
            // Retrieve the named panelMediator
            if (mediatorMap.TryRemove(mediatorName, out var mediator))
            {
                // for every notification this panelMediator is interested in...
                var interests = mediator.ListNotificationInterests();
                foreach (var interest in interests)
                {
                    // remove the observer linking the panelMediator 
                    // to the notification interest
                    RemoveObserver(interest, mediator);
                }

                // remove the panelMediator from the map
                mediator.OnRemove();
            }
            return mediator;
        }
