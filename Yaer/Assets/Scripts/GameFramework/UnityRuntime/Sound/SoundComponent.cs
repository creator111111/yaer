...
namespace GameFramework.UnityRuntime.Sound
{
    /// <summary>
    /// 声音组件。
    /// </summary>
    ...
    public sealed partial class SoundComponent : GameFrameworkComponent
    {
        ...
        [SerializeField]
        private string m_SoundHelperTypeName = "GameFramework.UnityRuntime.DefaultSoundHelper";

        [SerializeField]
        private SoundHelperBase m_CustomSoundHelper = null;

        [SerializeField]
        private string m_SoundGroupHelperTypeName = "GameFramework.UnityRuntime.DefaultSoundGroupHelper";

        [SerializeField]
        private SoundGroupHelperBase m_CustomSoundGroupHelper = null;

        [SerializeField]
        private string m_SoundAgentHelperTypeName = "GameFramework.UnityRuntime.DefaultSoundAgentHelper";
        ...
        public bool AddSoundGroup(string soundGroupName, bool soundGroupAvoidBeingReplacedBySamePriority, bool soundGroupMute, float soundGroupVolume, int soundAgentHelperCount)
        {
            ...
            soundGroupHelper.name = GameFramework.Utility.Text.Format("Sound Group - {0}", soundGroupName);
            ...
            {
                AudioMixerGroup[] audioMixerGroups = m_AudioMixer.FindMatchingGroups(GameFramework.Utility.Text.Format("Master/{0}", soundGroupName));
        ...
        /// <param name="bindingEntity">声音绑定的实体。</param>
        /// <returns>声音的序列编号。</returns>
        public int PlaySound(string soundAssetName, string soundGroupName, int priority, PlaySoundParams playSoundParams, Entity.Entity bindingEntity)
        {
        ...
        /// <param name="userData">用户自定义数据。</param>
        /// <returns>声音的序列编号。</returns>
        public int PlaySound(string soundAssetName, string soundGroupName, int priority, PlaySoundParams playSoundParams, Entity.Entity bindingEntity, object userData)
        {
        ...
        private bool AddSoundAgentHelper(string soundGroupName, SoundGroupHelperBase soundGroupHelper, int index)
        {
            ...
            soundAgentHelper.name = GameFramework.Utility.Text.Format("Sound Agent Helper - {0} - {1}", soundGroupName, index.ToString());
            Transform transform = soundAgentHelper.transform;
            transform.SetParent(soundGroupHelper.transform);
            transform.localScale = Vector3.one;

            var volumeChangedListener = transform.gameObject.AddComponent<VolumeChangedListener>();
            volumeChangedListener.SetSoundType((SoundType)Enum.Parse(typeof(SoundType), soundGroupName));

            if (m_AudioMixer != null)
            {
                AudioMixerGroup[] audioMixerGroups = m_AudioMixer.FindMatchingGroups(GameFramework.Utility.Text.Format("Master/{0}/{1}", soundGroupName, index.ToString()));
                if (audioMixerGroups.Length > 0)
                {
        ...
        {
            string logMessage = GameFramework.Utility.Text.Format("Play sound failure, asset name '{0}', sound group name '{1}', error code '{2}', error message '{3}'.", e.SoundAssetName, e.SoundGroupName, e.ErrorCode.ToString(), e.ErrorMessage);
