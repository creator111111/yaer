...
namespace GameFramework.UnityRuntime.Scene
{
    /// <summary>
    /// 场景组件。
    /// </summary>
    ...
    public sealed class SceneComponent : GameFrameworkComponent
    {
        ...
        private void RefreshSceneOrder()
        {
            if (m_SceneOrder.Count > 0)
            {
                ...
                UnityEngine.SceneManagement.Scene scene = SceneManager.GetSceneByName(GetSceneName(maxSceneName));
                if (!scene.IsValid())
                {
        ...
        {
            UnityEngine.SceneManagement.Scene lastActiveScene = SceneManager.GetActiveScene();
