        ...
        private sealed class ConsoleWindow : IDebuggerWindow
        {
            ...
            public void OnDraw()
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    m_InfoFilter = GUILayout.Toggle(m_InfoFilter, GameFramework.Utility.Text.Format("Info ({0})", m_InfoCount.ToString()), GUILayout.Width(90f));
                    m_WarningFilter = GUILayout.Toggle(m_WarningFilter, GameFramework.Utility.Text.Format("Warning ({0})", m_WarningCount.ToString()), GUILayout.Width(90f));
                    m_ErrorFilter = GUILayout.Toggle(m_ErrorFilter, GameFramework.Utility.Text.Format("Error ({0})", m_ErrorCount.ToString()), GUILayout.Width(90f));
                    m_FatalFilter = GUILayout.Toggle(m_FatalFilter, GameFramework.Utility.Text.Format("Fatal ({0})", m_FatalCount.ToString()), GUILayout.Width(90f));
                }
                ...
                GUILayout.BeginVertical("box");
                {
                    m_StackScrollPosition = GUILayout.BeginScrollView(m_StackScrollPosition, GUILayout.Height(100f));
                    {
                        if (m_SelectedNode != null)
                        {
                            ...
                            if (GUILayout.Button(GameFramework.Utility.Text.Format("<color=#{0}{1}{2}{3}><b>{4}</b></color>{6}{6}{5}", color.r.ToString("x2"), color.g.ToString("x2"), color.b.ToString("x2"), color.a.ToString("x2"), m_SelectedNode.LogMessage, m_SelectedNode.StackTrack, Environment.NewLine), "label"))
                            {
                                CopyToClipboard(GameFramework.Utility.Text.Format("{0}{2}{2}{1}", m_SelectedNode.LogMessage, m_SelectedNode.StackTrack, Environment.NewLine));
                            }
                        }
                    }
                    GUILayout.EndScrollView();
            ...
            private void OnLogMessageReceived(string logMessage, string stackTrace, LogType logType)
            {
                ...
                {
                    GameFramework.ReferencePool.Release(m_LogNodes.Dequeue());
                }
            }
            ...
            private string GetLogString(LogNode logNode)
            {
                ...
                return GameFramework.Utility.Text.Format("<color=#{0}{1}{2}{3}>[{4}][{5}] {6}</color>",
