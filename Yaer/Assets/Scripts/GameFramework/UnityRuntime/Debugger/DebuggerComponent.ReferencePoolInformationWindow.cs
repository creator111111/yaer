    ...
    public sealed partial class DebuggerComponent : GameFrameworkComponent
    {
        private sealed class ReferencePoolInformationWindow : ScrollableDebuggerWindowBase
        {
            ...
            protected override void OnDrawScrollableWindow()
            {
                ...
                {
                    DrawItem("Enable Strict Check", GameFramework.ReferencePool.EnableStrictCheck.ToString());
                    DrawItem("Reference Pool Count", GameFramework.ReferencePool.Count.ToString());
                }
                GUILayout.EndVertical();

                m_ShowFullClassName = GUILayout.Toggle(m_ShowFullClassName, "Show Full Class Name");
                m_ReferencePoolInfos.Clear();
                ReferencePoolInfo[] referencePoolInfos = GameFramework.ReferencePool.GetAllReferencePoolInfos();
                ...
                {
                    GUILayout.Label(GameFramework.Utility.Text.Format("<b>Assembly: {0}</b>", assemblyReferencePoolInfo.Key));
                    GUILayout.BeginVertical("box");
                    {
