...
using GameFramework.UnityRuntime.Network;
...
namespace GameFramework.UnityRuntime.Debugger
{
    public sealed partial class DebuggerComponent : GameFrameworkComponent
    {
        private sealed class NetworkInformationWindow : ScrollableDebuggerWindowBase
        {
            ...
            {
                GUILayout.Label(GameFramework.Utility.Text.Format("<b>Network Channel: {0} ({1})</b>", networkChannel.Name, networkChannel.Connected ? "Connected" : "Disconnected"));
                GUILayout.BeginVertical("box");
                {
                    ...
                    DrawItem("Send Packet", GameFramework.Utility.Text.Format("{0} / {1}", networkChannel.SendPacketCount.ToString(), networkChannel.SentPacketCount.ToString()));
                    DrawItem("Receive Packet", GameFramework.Utility.Text.Format("{0} / {1}", networkChannel.ReceivePacketCount.ToString(), networkChannel.ReceivedPacketCount.ToString()));
                    DrawItem("Miss Heart Beat Count", networkChannel.MissHeartBeatCount.ToString());
                    DrawItem("Heart Beat", GameFramework.Utility.Text.Format("{0} / {1}", networkChannel.HeartBeatElapseSeconds.ToString("F2"), networkChannel.HeartBeatInterval.ToString("F2")));
