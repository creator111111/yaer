...
namespace UnityGameFramework.Editor
{
    ...
    internal static class BuildSettings
    {
        ...
        private static List<string> s_DefaultSceneNames = new List<string>();
        private static List<string> s_SearchScenePaths = new List<string>();

        [InitializeOnLoadMethod]
        static void InitBuildSettings()
        {
            s_ConfigurationPath = Type.GetConfigurationPath<BuildSettingsConfigPathAttribute>() ?? Utility.Path.GetRegularPath(Path.Combine(Application.dataPath, "Scripts/GameFramework/Configs/BuildSettings.xml"));
            s_DefaultSceneNames.Clear();
            s_SearchScenePaths.Clear();

            if (!File.Exists(s_ConfigurationPath))
            { 
                Debug.LogWarning($"{s_ConfigurationPath} not exists");
                return;
            }
